function [band_var] = nevis_elevband_2010(fn,tis,terravel) % [uncomment to make a function ]
% nevis_elevband_2010(fn,tis,terravel)
% 
...
for i=1:length(t)
    ...
    ax(4) = subplot(4,1,4); 
        elev_vel = 50:100:1250;
        for k=12:20
        if isfield(terravel,'band_date'), 
        plot(elev_vel,terravel.band(k,:),'Color',c_time(terravel.band_date(k,2),:));
        hold on;
        xlabel('Elevation [ m ]'); xlim([0 1600]);
        ylabel('2010 V [ m^2/yr ]');
        end
        end

function nevis_qwindow_mean(fn,tis)
%% nevis discharge (q) 8 panel plot to go with Joughin 2013 velocities
%% LAS August 2016
...
%% determine wintertime average of 30 days values of Q
for i_s=10:40
...
for i_s=1:length(span)    
    ...
    vv2 = nevis_nodedischarge(vv2,aa,pp,gg,oo); % calculate node discharge
    nevis_unpack(aa,vv2);
    % save plotting variable q
    q_i_s(:,i_s) = qs+qe+qQ; 
    clear tt vv vv2
end
% average plotting variable over 11 days
qmean = nanmean(q_i_s,2);

%axes(axe1(i_t))   
    % discharge
    xx = (ps.x/10^3)*nx; yy = (ps.x/10^3)*ny;
    zz = ps.qs*reshape(qmean,gg.nI,gg.nJ);   
    cax = [10^(-5) 10^(0)]; 
...
%     text(-410,175,'q [ m^2 s^{-1} ]','FontSize',12,'Interpreter','tex');
%    
%    text(-550,180,([oo.root,oo.fn]),'FontSize',12,'Interpreter','tex');
       
%% save zz   
   zz_nevis(1,9).Q = reshape(Qwinter, size(xx,1), size(xx,2));
   zz_nevis(1,9).t = 365;
   save(num2str([fn,'_zz_nevisQ.mat']),'zz_nevis','-v7.3')
   
% RUN COHERENCE SCRIPTS FROM HERE
year = 2009;
[COH2, covar] = nevis_coherence_2009Q(fn, tis, zz_nevis); 
